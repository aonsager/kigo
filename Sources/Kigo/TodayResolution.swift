...
/// Designed for extension in slice #32: add `ko: Ko` and `sekki: Sekki` fields
...
public enum TodayResolver {
    ...
    /// or `nil` if the derived day-key is not present in `manifest.dailyMap`.
    ///
    /// - Parameters:
    ///   - date: The date to resolve (typically from a `DateProvider`).
    ///   - manifest: The loaded `Manifest` to look up.
    /// - Returns: A `ResolvedDay` carrying the `DailyMapEntry` for the date's `MM-DD` key,
    ///   or `nil` if the manifest has no entry for that key.
    public static func resolve(date: Date, manifest: Manifest) -> ResolvedDay? {
        guard let key = DayKey.make(from: date),
              let entry = manifest.dailyMap[key] else {
            return nil
        }
        return ResolvedDay(kigoEntry: entry)
    }
}
