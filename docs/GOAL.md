...
- **Project tooling:** multi-target Xcode project generated by XcodeGen from a
  ...
  - `xcodegen generate`
  - `xcodebuild test -scheme Kigo -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:<suite>`
...
- **StoreKit testing:** purchases are verified with the `StoreKitTest` framework
  against a committed `Kigo.storekit` configuration — no App Store Connect dependency.
...
- **Evidence:**
  ...
  3. Run `xcodebuild test -scheme Kigo -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:KigoTests/SmokeTests`
...
- **Evidence:**
  1. Run `xcodebuild test -scheme Kigo -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:KigoTests/ManifestValidationTests`
...
- **Evidence:**
  1. Run `xcodebuild test -scheme Kigo -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:KigoTests/ContentSourceTests`
...
- **Evidence:**
  1. Run `xcodebuild test -scheme Kigo -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:KigoTests/ResolutionTests`
...
- **Evidence:**
  1. Run `xcodebuild test -scheme Kigo -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:KigoUITests/TodayScreenUITests`
...
- **Evidence:**
  1. Run `xcodebuild test -scheme Kigo -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:KigoTests/EntitlementTests`
     — expect `** TEST SUCCEEDED **` and exit 0. Using `StoreKitTest` with the
     committed `Kigo.storekit`, the suite asserts:
     - with no transactions, the entitlement provider reports inactive;
     - after buying the `com.tomeitotameigo.kigo.widgets.monthly` product, it reports
       active and writes the active flag to the injectable shared store;
     - after clearing in-memory state and syncing transactions (restore), it reports
       active again.
...
- **Evidence:**
  1. Run `xcodebuild test -scheme Kigo -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:KigoWidgetTests/WidgetTimelineTests`
