...
///
/// All tests drive the model through an **injected in-memory fake** transaction
/// source, exactly as `EntitlementTests` does for `EntitlementProvider` itself.
/// There is no `SKTestSession`, no `buyProduct`, no `Kigo.storekit` wiring, and no
/// real StoreKit call — those paths hang under `xcodebuild` from the CLI
/// (ADR 0009 / CLAUDE.md).
@MainActor
final class PaywallTests: XCTestCase {
    ...
    func testRestoreLeavesIsActiveFalseWhenSourceReportsNothing() async {
        ...
        await model.loadState()
        // (the model starts false; we verify restore doesn't flip it incorrectly.)
